...
export class TechnologyGroupList extends CommonUIGroup {
  private branchesList?: UIGroupSlot<TechnologyBranchesList> =
    new UIGroupSlot();
  ...
  static build(
    ...
    for (let technology of Object.entries(technologies)) {
      ...
      const activation: ActivationCallback = () => {
        technologyGroupList.branchesList.populate(
          TechnologyBranchesList.build(scene, definition, descriptionBox)
        );
        technologyGroupList.disable();
      };

      const newButton = scienceSquare
        .addButton()
        .text("")
