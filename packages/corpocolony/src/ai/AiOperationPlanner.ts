...
export class AiOperationPlanner implements Activation {
  ...
  activate(): void {
    if (
      this.operationCapacity.getPercentage() >=
      expectedCapacityUtilizationPercentage
    ) {
      console.log(
        "Sufficient amount of operations are already running, do nothing"
      );
      return;
    }

    const newOperation = this.aiReconPlanner.planOperations(
      this.operationCapacity.getMissing()
    );
    if (!newOperation) {
      return;
    }
