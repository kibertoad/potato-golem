...
export class ButtonListBuilder {
  ...
  addButton(text: string, onClick?: () => void): void {
    // Create button image
    let buttonImage = this.scene.add.image(0, 0, this.textureKey)
      .setOrigin(0.5, 0.5)
      .setDisplaySize(this.width, this.height)
      .setInteractive();

    // Create text for the button
    let buttonText = this.scene.add.text(0, 0, text, {
      fontSize: '20px',
      color: '#ffffff'
    }).setOrigin(0.5, 0.5);

    // Create container for button and text
    let container = this.scene.add.container(this.currentX, this.currentY, [buttonImage, buttonText]);
